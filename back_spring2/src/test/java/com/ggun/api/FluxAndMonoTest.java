package com.ggun.api;

import com.lixga.api.common.config.CustomExceprion;
...
public class FluxAndMonoTest {
    @Mock
    CustomExceprion customExceprionMono;
    @Mock
    CustomExceprion customExceprionFlux;

    @BeforeEach
    void setup() {
        customExceprionMono = new CustomExceprion("Mono");
        customExceprionFlux = new CustomExceprion("Flux");
    }
    ...
    void FluxCreateTest() {
        Flux<Integer> flux = Flux.create((FluxSink<Integer> sink) -> {
            sink.onRequest(requst -> {
                for (int i = 0; i < requst + 3; i++) {
                    sink.next(i);
                }
//                sink.complete();
            });
        });
        System.out.println("FluxCreateTest : ");
