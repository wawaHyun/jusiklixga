package com.lixga.api.common.service.impl;

import com.lixga.api.common.model.PageDTO;
import com.lixga.api.common.service.PageService;
...
public class PageServiceImpl implements PageService {
        ...
        public PageDTO getPageDTO(int toTalPageSize, int pageNo) {
                ...
                int blockNumber = (pageNumber + pageSize) % pageSize == 0 ? ((pageNumber + pageSize) / pageSize) - 1
                ...
                pageDTO.setBlockCount(
                                pageDTO.getTotalCount() % (pageSize * BLOCK_SIZE) == 0
                                                ? totalCount / (pageSize * BLOCK_SIZE)
                                                : (totalCount / (pageSize * BLOCK_SIZE)) + 1);
                ...
                pageDTO.setEndRow(pageNumber != pageDTO.getPageCount() ? pageDTO.getStartRow() + pageSize - 1
                                : pageDTO.getStartRow() + (totalCount % pageSize));

                pageDTO.setStartPage((blockNumber - 1) * BLOCK_SIZE + 1);
                pageDTO.setEndPage(blockNumber != pageDTO.getBlockCount() ? startPage + BLOCK_SIZE - 1
                                : startPage + (totalCount % (pageSize * BLOCK_SIZE)) % 10);
